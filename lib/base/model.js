 ...
 *   Missing eager-loaded {@link Model#belongsTo belongsTo}, {@link Model#hasOne hasOne},
 *   {@link Model#morphOne morphOne}, and {@link Model#morphTo morphTo} relations are included with
 *   the value `null`.
 */
ModelBase.prototype.serialize = function(options) {
  ...
  if (options.shallow !== true) {
    let relations = _.mapValues(this.relations, (relation) => {
      ...
      if (typeof relation.isNew === 'function' && relation.isNew() && _.isEmpty(relation.attributes)) {
        return null;
      }
      return relation.toJSON ? relation.toJSON(options) : relation;
    });

    // omitNew already serializes new related models as null; drop those keys.
    if (options.omitNew) {
 ...
 * @returns {Model|Collection|null|undefined}
 *   The specified relation as defined by a method on the model, `null` if a single relation was
 *   eager loaded and no related row exists, or `undefined` if the relation is not defined.
 */
ModelBase.prototype.related = function(name) {
  if (Object.prototype.hasOwnProperty.call(this.relations, name)) {
    return this.relations[name];
  }
  return this[name] ? (this.relations[name] = this[name]()) : void 0;
};
...
ModelBase.prototype.clone = function() {
  ...
  Object.assign(
    model.relations,
    _.mapValues(this.relations, (r) => (r && typeof r.clone === 'function' ? r.clone() : r))
  );
