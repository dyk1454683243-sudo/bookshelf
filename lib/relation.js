  ...
  /** @lends Relation.prototype */ {
    ...
    eagerPair(relationName, related, parentModels, options) {
      ...
      _.each(parentModels, (model) => {
        ...
        const matched = groupedKey != null ? grouped[groupedKey] : undefined;
        const hasMatch = Boolean(matched && matched.length);

        // Missing belongsTo / hasOne / morphOne / morphTo relations serialize as null, not {}.
        if (this.isSingle() && !hasMatch) {
          model.relations[relationName] = null;
          return;
        }

        if (groupedKey != null) {
          const relation = (model.relations[relationName] = this.relatedInstance(matched, options));
          if (this.type === 'belongsToMany') {
