...
const Relation = RelationBase.extend(
  /** @lends Relation.prototype */ {
    ...
    eagerPair(relationName, related, parentModels, options) {
      ...
      // Loop over the `parentModels` and attach the grouped sub-models, binding relatedData
      // to each parent so later save/refresh queries have a defined parentFk.
      _.each(parentModels, (model) => {
        ...
        if (groupedKey != null) {
          ...
          // Recreate relatedData from this parent. Collection eager loads initialize the shared
          // Relation from a blank target, so parentFk is unset. Saving the related model then
          // refreshes with `where <foreign_key> = ?` and an undefined binding (#2046).
          // belongsToMany already required this so attach() after fetchAll has a parent (#629).
