...
const BookshelfModel = ModelBase.extend(
  {
     ...
     *   Whether to enable debugging mode or not. When enabled will show information about the
     *   queries being run.
     * @param {string} [options.withSchema]
     *   Schema (or MySQL database) name passed through to Knex for the save query and, when
     *   auto-refresh runs, the follow-up {@link Model#refresh refresh} fetch.
        ...
        .then(function(method) {
          ...
          return this.triggerThen(
            ...
            .then(function(resp) {
              ...
              // Keep query options on auto-refresh. Dropping withSchema here
              // produces an unqualified SELECT after a schema-qualified INSERT
              // (ER_NO_DB_ERROR / "No database selected") — #2047.
              return this.refresh({
                silent: true,
