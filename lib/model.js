...
// Clone a knex query only when it already has WHERE constraints. Used so that
// `new Model().where(...).save()` can auto-refresh without an id (#2086).
function cloneConstrainedQuery(query) {
  if (!query || typeof query.clone !== 'function' || !query._statements) return null;
  if (_.filter(query._statements, {grouping: 'where'}).length === 0) return null;
  return query.clone();
}
...
const BookshelfModel = ModelBase.extend(
  {
    ...
    refresh(options = {}) {
      ...
      // `constrainByQuery` is used by save() after `.where(...).save()` so the
      // row is found by the original WHERE clause instead of incomplete
      // in-memory attributes (#2086).
      if (this.isNew() && !options.constrainByQuery) {
     ...
     *       })
     *
     *     // Same update using query constraints instead of model attributes:
     *     // UPDATE authors SET "bio" = 'Short user bio' WHERE "id" = 1 AND "site_id" = 2
     *     new Author()
     *       .where({id: 1, site_id: 2})
     *       .save({bio: 'Short user bio'}, {patch: true})
     *       .then((model) => {
     *         // ...
     *       })
      ...
      return Promise.bind(this)
        ...
        .then(function(method) {
          ...
          return this.triggerThen(
            ...
            .bind(this)
            .then(function() {
              // Capture WHERE constraints before the builder is executed. After
              // sync() the model query is reset, so a later refresh() cannot
              // find the row if the instance has no id (#2086).
              if (method === 'update' && this.id == null) {
                queryForRefresh = cloneConstrainedQuery(sync.query);
              }
            ...
            .then(function(resp) {
              ...
              // `new Model().where(...).save({patch: true})` has no id to refresh
              // by. Reuse the update's WHERE constraints instead of the
              // (possibly incomplete) in-memory attributes (#2086).
              if (queryForRefresh) {
                this._knex = queryForRefresh;
                return this.refresh({
                  silent: true,
